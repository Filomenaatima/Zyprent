...
function parseShares(value: string) {
...
function estimateYield(item: OpportunityItem) {
  return Number((8 + item.progress * 0.03).toFixed(1));
}

function estimateUpside(item: OpportunityItem) {
  return item.progress >= 70 ? 18 : item.progress >= 40 ? 13 : 10;
}
  ...
  const investorCount = useMemo(() => {
    const ids = new Set(
      filteredInvestments.map(
        (item) => item.investor?.email || item.investor?.name || item.id,
      ),
    );
    return ids.size;
  }, [filteredInvestments]);
    ...
    <div className="investments-shell">
      <section className="investments-hero">
        <div className="investments-hero-copy">
          ...
          <h1 className="investments-title">
            Monitor capital deployment, investor participation, and funding
            performance
          </h1>
        ...
        <div className="investments-summary-grid">
          ...
          <div className="investments-summary-card">
            ...
            <h3 className="investments-summary-value small">
              {formatCurrency(overview?.totalDistributed ?? 0)}
            </h3>
          </div>
      ...
      <section className="investments-overview-bar">
        ...
        <div className="investments-overview-item">
          ...
          <strong>{formatCurrency(averageInvestment)}</strong>
        </div>
        <div className="investments-overview-item">
        ...
        <div className="investments-section-head">
          <div>
            ...
            <p className="investments-section-subtitle">
              Platform-wide visibility into investor allocations and capital
              movements
            </p>
          </div>
            ...
            {filteredInvestments.map((item) => (
              <div key={item.id} className="investments-history-card premium">
                ...
                <div className="investments-history-mid">
                  <span className="investments-history-type buy">
                    INVESTMENT
                  </span>
                  <span>
                    {item.investor?.name ||
              ...
              >
                ...
                <div className="investments-signal-row">
                  <div className="investments-signal-card">
                    ...
                    <strong>{formatCurrency(item.amount / item.count)}</strong>
                  </div>
                  <div className="investments-signal-card">
...
function InvestorInvestmentsView() {
  ...
  const [selectedOffer, setSelectedOffer] = useState<OpportunityItem | null>(null);
  ...
  const selectedShares = parseShares(buyShares);
  const selectedTotal = selectedOffer
    ? selectedShares * Number(selectedOffer.pricePerShare || 0)
    : 0;
  ...
  const toggleCompare = (propertyId: string) => {
    setCompareIds((prev) => {
      ...
      if (prev.length >= 3) return [...prev.slice(1), propertyId];
      return [...prev, propertyId];
    });
  };

  const openBuyModal = (item: OpportunityItem) => {
  ...
  const submitBuyShares = async () => {
    if (!selectedOffer) return;

    const shares = parseShares(buyShares);
    ...
    if (shares > selectedOffer.sharesRemaining) {
      setActionError(
        `Only ${selectedOffer.sharesRemaining} shares remain for this offer.`,
      );
      return;
    }

    try {
      setBuyingPropertyId(selectedOffer.propertyId);
      ...
      setActionMessage(
        `Successfully purchased ${shares} share${shares > 1 ? "s" : ""} in ${
          selectedOffer.title
        }.`,
      );
  ...
  const handleBuyListing = async (item: MarketplaceItem) => {
    ...
    try {
      ...
      setActionMessage(
        `Successfully purchased ${shares} listed share${
          shares > 1 ? "s" : ""
        } from ${item.property.title}.`,
      );
    ...
    <>
      <div className="investments-shell">
        <section className="investments-hero">
          ...
          <div className="investments-summary-grid">
            ...
            <div className="investments-summary-card">
              ...
              <h3 className="investments-summary-value small">
                {formatCurrency(summary.totalCapitalOpen)}
              </h3>
            </div>
          </div>
        </section>
        ...
        <nav className="investments-quick-nav">
          <button type="button" className="investments-quick-pill" onClick={() => scrollToSection(offersRef)}>
            Offers
          </button>
          <button type="button" className="investments-quick-pill" onClick={() => scrollToSection(marketplaceRef)}>
            Marketplace
          </button>
          <button type="button" className="investments-quick-pill" onClick={() => scrollToSection(historyRef)}>
            History
          </button>
          <button type="button" className="investments-quick-pill" onClick={() => scrollToSection(archiveRef)}>
            Archive
          </button>
        </nav>
        ...
        <section ref={offersRef} className="investments-section">
          ...
          <div className="investments-toolbar">
            <div className="investments-toolbar-left">
              {(["all", "watchlist", "holdings", "open-only"] as OfferFilter[]).map(
                (filter) => (
                  <button
                    key={filter}
                    type="button"
                    className={`investments-filter-pill ${
                      offerFilter === filter ? "active" : ""
                    }`}
          ...
          {loading ? (
            ...
            <div className="investments-card-grid">
              {filteredOffers.map((item) => {
                ...
                return (
                  <div key={item.propertyId} className="investments-card premium elevated">
                    <div className="investments-card-top">
                      <div>
                        <div className="investments-card-topline">
                          <span className="investments-location-tone">
                      ...
                      <span className={`investments-status ${getOfferStatus(item).toLowerCase().replace(" ", "-")}`}>
                        {getOfferStatus(item)}
                      </span>
                    </div>
                    ...
                    <div className="investments-price-block">
                      <div>
                        ...
                        <p className="investments-price-caption">Price per share</p>
                      </div>
                    ...
                    <div className="investments-signal-row">
                      <div className="investments-signal-card">
                        <span>Yield</span>
                        <strong>{estimateYield(item)}% / yr</strong>
                      </div>
                      <div className="investments-signal-card">
                        <span>Upside</span>
                        <strong>{estimateUpside(item)}%</strong>
                    ...
                    <div className="investments-metrics-grid">
                      <div className="investments-metric-card">
                        ...
                        <p className="investments-metric-value">{item.totalShares}</p>
                      </div>

                      <div className="investments-metric-card">
                        <p className="investments-metric-label">Shares Sold</p>
                        <p className="investments-metric-value">{item.sharesSold}</p>
                      </div>
                    ...
                    <div className="investments-card-actions-top">
                      <button
                        type="button"
                        className={`investments-ghost-button ${onWatchlist ? "active" : ""}`}
                      ...
                      <button
                        type="button"
                        className={`investments-ghost-button ${inCompare ? "active" : ""}`}
        ...
        <section ref={marketplaceRef} className="investments-section">
          ...
          {marketplace.length === 0 ? (
            <div className="investments-empty">
              ...
              <br />
              <span>Check back later for investor-to-investor opportunities.</span>
            </div>
          ) : (
            <div className="investments-market-grid">
              {marketplace.map((item) => (
                <div key={item.id} className="investments-market-card premium-market">
                  <div className="investments-market-top">
                    ...
                    <span className="investments-market-badge">Live Listing</span>
                  </div>
        ...
        <section ref={historyRef} className="investments-section">
          <div className="investments-section-head">
            <div>
              ...
              <p className="investments-section-subtitle">
                A clean record of your share activity across all investment positions
              </p>
            </div>
            ...
            <div className="investments-history-list">
              {history.map((item) => (
                <div key={item.id} className="investments-history-card premium">
                  ...
                  <div className="investments-history-mid">
                    <span className={`investments-history-type ${getHistoryTone(item.type)}`}>
                      {item.type}
                    </span>
        ...
        <div className="resident-payment-modal-backdrop">
          <div className="resident-payment-modal">
            <div className="resident-payment-modal-head">
              <div>
                <p className="resident-payment-modal-eyebrow">Buy shares</p>
              ...
              <button
                ...
                disabled={buyingPropertyId === selectedOffer.propertyId}
              >
                ×
              </button>
            </div>
            ...
            <div className="resident-payment-modal-summary">
              <div className="resident-payment-modal-summary-card">
                ...
                <strong>{formatCurrency(selectedOffer.pricePerShare)}</strong>
                <p>{selectedOffer.location || "No location"}</p>
              </div>

              <div className="resident-payment-modal-summary-card">
                <span>Shares remaining</span>
                <strong>{selectedOffer.sharesRemaining}</strong>
                <p>{selectedOffer.progress}% funded</p>
              </div>

              <div className="resident-payment-modal-summary-card">
                <span>Total purchase</span>
                <strong>{formatCurrency(selectedTotal)}</strong>
                <p>Deducted from wallet after confirmation.</p>
              </div>
            </div>

            <div className="resident-payment-modal-section">
              <label className="resident-payment-label">Number of shares</label>
              <input
            ...
            <div className="resident-payment-modal-footer">
              ...
              <button
                ...
                onClick={submitBuyShares}
                disabled={buyingPropertyId === selectedOffer.propertyId}
              >
                {buyingPropertyId === selectedOffer.propertyId
                  ? "Processing..."
                  : "Confirm purchase"}
              </button>
            </div>
